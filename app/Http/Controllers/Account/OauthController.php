...
class OauthController extends Controller
{
    ...
    {
        $product_type = $request->input('product_type', 'wap');

        $clientId                 = env('WEIXIN_KEY');
        $clientSecret             = env('WEIXIN_SECRET');
        $redirectUrl              = url('/oauth/weixin/callback');
        $additionalProviderConfig = ['product_type' => $product_type];
    ...
    public function callback($type, Request $request, Guard $auth)
    {
        ...
        // 获取授权登陆用户信息
        $oauthUser = Socialite::driver($type)->setConfig($config)->user();
    ...
    public function callbackApi($type, Request $request, Guard $auth)
    {
        // 获取授权登陆用户信息
        $oauthUser = Socialite::driver($type)->user();
        print_r($oauthUser);
